...
다음 단계 후보로 [edwardkim/rhwp](https://github.com/edwardkim/rhwp)의 HWP 파서·편집기와 `GetFieldList`/`PutFieldText` 누름틀 API를 검토합니다. 목표는 승인된 `.hwp` 보고서 양식을 Field Desktop에서 열고, 태블릿 인계 값을 이름 있는 누름틀에 채운 뒤 **원본을 덮어쓰지 않고 새 HWP 파일로 저장**하는 것입니다. `rhwp`는 아직 0.x 개발 단계이므로 현재 설치본에는 포함하지 않으며, 실제 발굴보고서 양식의 표·사진·머리말·쪽 나눔을 한컴오피스에서 다시 열어 비교하는 회귀 검증을 통과한 뒤에만 선택 기능으로 추가합니다. 그전까지는 위의 일반 텍스트 복사를 기본 경로로 유지합니다.
...
Field Desktop의 화면 다듬기는 [emilkowalski/skills](https://github.com/emilkowalski/skills)의 즉시 반응, 단순한 기본 경로, 명확한 정보 위계, 절제된 모션 원칙을 참고합니다. 발굴 업무 화면에 불필요한 장식 효과는 가져오지 않고, 많은 기록을 찾고 검수하고 복사하는 속도를 높이는 원칙만 적용합니다.
...
자료가 많이 쌓여도 모두 펼쳐 놓지 않습니다. 왼쪽 검색에서 유구명이나 자료명을 찾고 짧은 기록 목록에서 하나를 고르면 오른쪽에 그 기록의 HWP 본문과 검수 내용만 나타납니다. 사진·토층사진·도면·메모 같은 태블릿 자료 묶음과 전체 복사 미리보기는 기본적으로 접혀 있습니다. 작은 노트북 화면에서는 목록과 상세 작업대가 위아래로 바뀝니다.
...
태블릿을 USB 디버깅으로 연결하고 저장할 때마다 바로 확인하려면 저장소 루트의 `START_TABLET_LIVE.cmd`를 실행합니다. 개발 앱은 일반 현장 앱과 별도 패키지로 설치되므로 기존 현장 앱과 그 자료를 덮어쓰지 않습니다.

1. 처음 한 번 또는 네이티브 모듈을 바꾼 뒤에는 `START_TABLET_LIVE.cmd --install-debug`를 실행합니다.
2. 평소에는 `START_TABLET_LIVE.cmd`만 실행합니다.
3. `mobile` 또는 `core`의 코드를 수정하고 저장하면 Metro의 Fast Refresh로 태블릿에 반영됩니다.
4. 화면이 예전 상태로 남을 때만 `START_TABLET_LIVE.cmd --clear`를 사용합니다.

Node, npm, Gradle, Android 빌드 산출물은 `IDAI_FIELD_DEV_ROOT`가 있으면 그 위치를 사용하고, 없으면 현재 개발 컴퓨터의 `H:\idai-field-dev`를 사용합니다. GitHub Actions로 배포용 APK를 새로 설치하는 기존 흐름은 여러 기기에 검증본을 전달할 때 사용합니다.
