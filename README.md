...
태블릿에서 Field Hub 동기화를 실행하면 기록 JSON과 현장 사진 원본을 같은 프로젝트 자료로 올립니다. 사진 원본은 Field Hub의 `original_image` 파일 API로 보존하고, 각 기록에는 `fieldworkImageUploadStatus`, `fieldworkImageUploadedAt`, `fieldworkImageUploadTarget` 같은 업로드 감사 필드가 남습니다.

Field Desktop에서는 같은 프로젝트를 다운로드하거나 동기화한 뒤, 사진 원본이 내려왔는지와 `보고서/HWP 복사` 패널의 근거 자료가 채워졌는지 확인합니다. 보고서 사진 인계를 별도로 묶을 때는 Field Desktop의 이미지 내보내기가 `fieldwork-image-export-manifest.json`, `fieldwork-image-export-manifest.csv`, `fieldwork-image-export-readme.txt`를 함께 만들어 원본 파일명, 태블릿 표시, 토층사진 스포이드 위치를 대조할 수 있게 합니다.
