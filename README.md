...
보고서 작성 보조 기능의 기본 위치도 Field Desktop 안입니다. 현재 Field Desktop의 `보고서/HWP 복사` 패널은 태블릿 기록을 검수한 뒤 `본문 복사`, `복사`, 섹션별 복사로 HWP에 붙여넣기 쉬운 일반 텍스트를 제공합니다. `tools\bridgedesk`에 있는 BridgeDesk는 별도 제품이 아니라, HWP 복사 문장과 표 형식을 빠르게 검증하기 위한 임시 시제품입니다.
...
4. 작성 중인 HWP 본문에는 `본문 복사`를 우선 사용하고, 근거·확인 항목까지 함께 옮길 때만 `복사`나 섹션별 복사를 사용합니다. 클립보드에는 HWP 양식이 덜 흐트러지도록 일반 텍스트 클립보드 내용이 들어갑니다.
5. 보고서 보조 시제품이 필요한 경우에만 태블릿 JSON 파일을 `tools\bridgedesk\data\inbox` 폴더에 넣어 별도 검증합니다.

`data\inbox`가 비어 있으면 BridgeDesk는 발굴 현장 예시 데이터인 `tools\bridgedesk\data\tablet_submissions.json`을 읽어 보고서 흐름을 먼저 확인합니다. BridgeDesk에서 새로 검증한 HWP 문장 형식은 Field Desktop 안의 보고서/HWP 패널에 맞추는 것을 기준으로 합니다.

## 태블릿 자료를 데스크톱으로 받기

태블릿에서 Field Hub 동기화를 실행하면 기록 JSON과 현장 사진 원본을 같은 프로젝트로 올립니다. 사진 원본은 Field Hub의 `original_image` 파일 API로 보존하고, 각 기록에는 `fieldworkImageUploadStatus`, `fieldworkImageUploadedAt`, `fieldworkImageUploadTarget` 같은 업로드 감사 필드가 남습니다.

Field Desktop에서는 같은 프로젝트를 다운로드/동기화한 뒤, 사진 원본이 내려왔는지와 `보고서/HWP 복사` 패널의 근거 자료가 채워졌는지 확인합니다. 보고서 사진 인계를 별도로 묶을 때는 Field Desktop의 이미지 내보내기가 `fieldwork-image-export-manifest.json`, `fieldwork-image-export-manifest.csv`, `fieldwork-image-export-readme.txt`를 함께 만들어 원본 파일명, 태블릿 표시, 토층사진 스포이드 위치를 대조할 수 있게 합니다.
...
이 fresh build 명령은 실행 전에 로컬 작업트리가 깨끗한지, 현재 로컬 커밋이 GitHub의 빌드 대상 `master`와 같은지 확인합니다. 아직 커밋하지 않은 수정이나 푸시하지 않은 커밋이 있으면 태블릿 설치를 멈춥니다. 의도적으로 원격 `master`만 빌드하려면 `-AllowRefMismatch`를 추가합니다.
