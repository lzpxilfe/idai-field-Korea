...
| 태블릿 APK 설치 | `install-idai-field-android-apk.ps1` | Android 태블릿에 현장 기록 앱 설치 |
...
3. Field Desktop에서 같은 프로젝트를 열어 자료를 검수합니다.
4. 보고서 보조 시제품이 필요한 경우 JSON 파일을 `tools\bridgedesk\data\inbox` 폴더에 넣습니다.
5. HWP 문장과 표 형식을 확인한 뒤, 검증된 흐름을 Field Desktop의 보고서 패널로 옮깁니다.

`data\inbox`가 비어 있으면 BridgeDesk는 발굴 현장 예시 데이터인 `tools\bridgedesk\data\tablet_submissions.json`을 읽어 보고서 흐름을 먼저 확인합니다.
